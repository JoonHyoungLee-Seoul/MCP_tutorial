...
def print_tool_info(chunk_msg):
    """도구 호출 정보를 출력하는 헬퍼 함수"""
    # AIMessage의 tool_calls 확인
    if isinstance(chunk_msg, AIMessage) and hasattr(chunk_msg, 'tool_calls') and chunk_msg.tool_calls:
        for tool_call in chunk_msg.tool_calls:
            tool_name = tool_call.get('name', 'Unknown')
            print(f"\n🔧 Tool Selected: \033[1;33m{tool_name}\033[0m")

    # ToolMessage 확인
    elif isinstance(chunk_msg, ToolMessage):
        tool_name = chunk_msg.name if hasattr(chunk_msg, 'name') else 'Unknown'
        print(f"\n✅ Tool Executed: \033[1;32m{tool_name}\033[0m")

    # 리스트 형태의 메시지 처리
    elif isinstance(chunk_msg, list):
            ...
            if isinstance(node_chunks, dict):
                for node_name, node_chunk in node_chunks.items():
                    ...
                    # 콜백이 없는 경우 기본 출력
                        ...
                        # 도구 호출 정보 출력 (node_chunk에서 메시지 추출)
                        if isinstance(node_chunk, dict):
                            for k, v in node_chunk.items():
                                if isinstance(v, (AIMessage, ToolMessage)):
                                    print_tool_info(v)
